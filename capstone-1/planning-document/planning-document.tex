...
%				why polynomial > exponential
			...
			\subsection{Understanding the Proof}
			
				Proving that a problem is NP-complete involves reductions. As mentioned in Subsection \ref{subsec:compclass}, if all problems in NP can be reduced to problem $X$, $X$ is said to be NP-hard. Similarly, proving a problem is NP-complete involves reducing a known NP-complete problem to the problem itself. If problem A is NP-complete and can be reduced to problem B, problem B is NP-complete.
				
				% 
		
		\section{Addressing NP-complete Problems}
		
			% Explain the different appro\part{title}aches taken to solving NP-complete problems
